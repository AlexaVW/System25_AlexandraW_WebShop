    ...
    internal class Create
    {
        ...
        public static void CreateProduct() 
        {
            using (var db = new WebShopDbContext())
            {
                ...
                bool validStockInput = int.TryParse(Console.ReadLine(), out int unitsInStock);
                ...
                bool isOnSale = false;
                bool validOnSaleInput = int.TryParse(Console.ReadLine(), out int onSale);
                // Any other input than 1 = false
                if (onSale == 1)
                {
                    isOnSale = true;
                }
                ...
                bool validCategoryId = int.TryParse(Console.ReadLine(), out int categoryId);
        ...
        // Adding a product to cart (Customer)
        public static void CreateCartItem(Product product)
        {
            using (var db  = new WebShopDbContext())
            {
                int productAmount = 1; // Amount always starts with 1 when adding to cart
                bool isPaid = false; // isPaid is always false until after payment of the cart item
                int productId = product.Id;
                
                CartItem newCartItem = new CartItem(productAmount, isPaid, productId);
                
                // If the product with the same id is already in cart = Add +1 to ProductAmount
                // Else = Add one item to cart
        ...
        public static void CreateOrder()
        {
            ...
            List<CartItem> cartItemsNotPaid = Helpers.GetCartItemsNotPaid();
            ...
            using (var db = new WebShopDbContext())
            {
                ...
                bool validShippingInput = int.TryParse(Console.ReadLine(), out int selectedShipping);
        ...
        private static void Pay(List<Order> orders, int shippingCost)
        {
        ...
        private static string ChoosePaymentMethod()
        {
            ...
            bool validPaymentInput = int.TryParse(Console.ReadLine(), out int selectedPayment);
        ...
        private static void ShowCheckoutInformation(string paymentMethod, double totalCartPrice, int shippingCost)
        {
            Console.WriteLine("Payment method: " + paymentMethod);
            Console.WriteLine("CartItem price: " + totalCartPrice + " SEK");
            Console.WriteLine("Shipping: " + shippingCost + " SEK");
            Console.WriteLine("Subtotal: " + (totalCartPrice + shippingCost).ToString("N2") + " SEK");
        }

        private static void ContinueToPay(List<Order> orders, int shippingCost)
        {
            Console.WriteLine("Press [1] to continue");
            bool validInput = int.TryParse(Console.ReadLine(), out int pay); 
            if (pay == 1)
            {
                Console.Clear();
                // Sending the orders and shipping cost to Pay-method
        ...
        private static void ConfirmPayment(List<Order> orders, WebShopDbContext db)
        {
            ...
            if (pay == 1)
            {
                ...
                ShowMessageAndGoToHomePage(message);
            }
        }

        private static void ShowMessageAndGoToHomePage(string message)
        {
            Console.WriteLine("\n" + message); // message depends on if the payment went through or not
        ...
        private static List<Order> AddOrderToList(string name, string address, string country, string shippingMethod, string paymentMethod, WebShopDbContext db, List<CartItem> cartItems)
        {
